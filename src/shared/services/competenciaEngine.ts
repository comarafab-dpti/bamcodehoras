...
export function validarLancamentoCanteiro(
  ...
  // Ausência de controle do mês anterior = LIBERADO (primeiro mês do sistema,
  // canteiro novo, controle ainda não criado ou listener ainda carregando).
  // Nunca bloquear por falta de registro — bloqueia somente quando o controle
  // existe E o canteiro está explicitamente ABERTO/REABERTO.
  if (!statusCanteiros || statusCanteiros[canteiro] === undefined) {
    return {
      permitido: true,
      bypass: false,
      motivo: 'Sem controle de fechamento do mês anterior. Lançamento liberado.',
    };
  }

  if (statusCanteiros[canteiro]?.status === 'FECHADO') {
    return {
      permitido: true,
      bypass: false,
