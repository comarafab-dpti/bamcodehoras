...
// Ausência de controle (primeiro mês, canteiro novo, listener carregando) = LIBERADO
assert(validarLancamentoCanteiro(undefined, 'KO').permitido, 'libera quando nao existe controle do mes anterior');
assert(validarLancamentoCanteiro({}, 'KO').permitido, 'libera quando o mapa de statusCanteiros esta vazio');
const semCanteiroNoMapa: StatusCanteiros = {
  BE: { status: 'FECHADO', data: '2026-08-31T18:00:00Z', usuario: 'admin@comara.gov.br', motivo: null },
};
assert(validarLancamentoCanteiro(semCanteiroNoMapa, 'KO').permitido, 'libera canteiro sem entrada no mapa de controle');
assert(validarLancamentoCanteiro(semCanteiroNoMapa, 'BE').permitido, 'canteiro presente e fechado permanece liberado');
